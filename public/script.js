...
// ============================================
// LOGIC CẦU GIỌT NƯỚC (BEAD PLATE)
// ============================================
function renderBeadPlate(res) {
    ...
    for(let i=0; i<fillCount; i++) { html += `<div class="bead-cell"></div>`; }
...
function drawWaveChart() {
    ...
    chartPoints.forEach((p, i) => {
        ...
        if(i===0) ctxChart.moveTo(x, y);
            ...
            let cpX = (prevX + x) / 2;
            ctxChart.quadraticCurveTo(prevX, prevY, cpX, (prevY+y)/2); 
            ctxChart.lineTo(x, y);
        }
    });
